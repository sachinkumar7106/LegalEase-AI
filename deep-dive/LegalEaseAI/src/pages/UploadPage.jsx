...
export default function UploadPage({ onNav }) {
  const { token } = useAuth();
                   ...
                   <div className="w-full h-full rounded-sm bg-white p-12 text-black overflow-y-auto font-serif">
                     {activeFile.analysis ? (
                       <div className="space-y-6 opacity-70">
                         ...
                         {activeFile.analysis.important_clauses?.map((c, i) => (
                    ...
                    <AnimatePresence mode="wait">
                      {activeTab === "Summary" && (
                        <motion.div key="Summary" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
                          <section>
                            ...
                            <p className="text-sm leading-relaxed text-foreground/90 bg-card border border-border p-4 rounded-xl">
                              {activeFile.analysis.document_overview}
                            </p>
                          </section>
                          
                          {activeFile.analysis.clause_tags?.length > 0 && (
                            <section>
                              <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-4">Clause Ontology</h4>
                              <div className="flex flex-wrap gap-2">
                                 {activeFile.analysis.clause_tags.map((t, i) => (
                      ...
                      {activeTab === "Risks" && (
                        <motion.div key="Risks" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
                           ...
                           {activeFile.analysis.risk_summary?.length > 0 ? (
                      ...
                      {activeTab === "Suggestions" && (
                        <motion.div key="Suggestions" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
                           ...
                           {activeFile.analysis.important_clauses?.length > 0 ? (
                           ...
                           <Button className="w-full mt-4" onClick={() => onNav("chat", { context: activeFile.analysis, docName: activeFile.name })}>Generate Revisions in Chat</Button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
