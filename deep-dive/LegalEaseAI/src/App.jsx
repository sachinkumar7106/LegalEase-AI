...
function AuthenticatedApp() {
  ...
  const handleNav = (newPage, data = null) => {
    setPage(newPage);
    setPageData(data);
  };
    ...
    <div className="flex h-screen bg-background overflow-hidden text-foreground font-sans selection:bg-primary/20">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-1 overflow-x-hidden">
          ...
          <NavItem icon={Home} label="Overview" page="landing" current={page} onClick={handleNav} collapsed={collapsed} />
          <NavItem icon={LayoutDashboard} label="Dashboard" page="dashboard" current={page} onClick={handleNav} collapsed={collapsed} />
          ...
          <NavItem icon={MessageSquare} label="AI Assistant" page="chat" current={page} onClick={handleNav} collapsed={collapsed} />
          <NavItem icon={FileBox} label="Documents" page="upload" current={page} onClick={handleNav} collapsed={collapsed} />
          <NavItem icon={Briefcase} label="Cases" page="cases" current={page} onClick={handleNav} collapsed={collapsed} />
        </div>
          ...
          <header className="h-16 shrink-0 border-b border-border bg-background/90 backdrop-blur-sm sticky top-0 z-10 px-8 flex items-center justify-between shadow-sm">
            ...
            <div className="flex items-center gap-3">
              <Button 
                variant="outline" 
                size="sm" 
                className="gap-2 shrink-0 border-border shadow-sm"
                onClick={() => toast("All systems operational. No new alerts.")}
              >
                <Bell size={14} /> Alerts
              </Button>
              {page === "dashboard" && (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Suspense fallback={<PageLoader />}>
                ...
                {page === "chat" && <ChatPage pageData={pageData} onNav={handleNav} />}
                {page === "upload" && <UploadPage onNav={handleNav} />}
